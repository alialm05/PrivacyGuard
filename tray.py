...
def _make_icon_image() -> Image.Image:
    """Draw a simple shield icon for the system tray."""
    size = 64
...
def _build_window():
    ...
    # ── Watch Folder ────────────────────────────────────────────────────────
    tk.Label(
        body, text="Watch Folder",
        font=("Segoe UI", 10, "bold"), anchor="w"
    ).grid(row=1, column=0, sticky="w", pady=(0, 3))

    folder_var = tk.StringVar(value=config.get("watch_folder"))
    folder_frame = tk.Frame(body)
    folder_frame.grid(row=2, column=0, sticky="ew", pady=(0, 14))

    tk.Entry(folder_frame, textvariable=folder_var, width=40).pack(side="left", padx=(0, 6))
    tk.Button(
        folder_frame, text="Browse…",
        command=lambda: folder_var.set(
            filedialog.askdirectory(title="Select folder to watch") or folder_var.get()
        )
    ).pack(side="left")
    ...
    archive_frame.grid(row=5, column=0, sticky="ew", pady=(0, 10))
    ...
    def on_save():
        config.set("watch_folder", folder_var.get())
        config.set("archive_path", archive_var.get())
        config.set_archive_password(pw_var.get())
        config.save()
        messagebox.showinfo(
            "Saved",
            "Settings saved.\n\nNote: changing the watch folder takes effect after restarting the app.",
            parent=root
        )
    ...
    tk.Button(
        ...
        bg="#27ae60", fg="white", padx=14, pady=4, font=("Segoe UI", 9, "bold")
    ).pack(side="left")
